...
import { ChevronDown, PlayIcon, RotateCcwIcon, RefreshCwIcon, Clapperboard } from 'lucide-react';
...
type ProjectWithVideoUrl = ReturnType<typeof useQuery<any>>[number];
...
export function ProjectManager() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Failed to start video creation. Check the console for details.');
    }
  };
          ...
          {projects?.map(project => (
            <Card key={project._id} className="overflow-visible relative">
              <div className="flex justify-between">
                      ...
                      {project.urls && project.urls.length > 0 && (
                        <div>
                          ...
                          <ul className="list-disc pl-5">
                            {project.urls.map((url, i) => <li key={i} className="truncate"><a href={url} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">{url}</a></li>)}
                          </ul>
                        </div>
                      )}
                    </div>
                        ...
                        <CollapsibleContent>
                          <div className="space-y-2 text-sm text-gray-400">
                            {project.plan.map(scene => (
                              <div key={scene.sceneNumber} className="p-2 border rounded">
                                  ...
                                  dialogue.voiceUrl && (
                                    <div key={i} className="my-1">
                                      <div className="flex items-center space-x-2">
                                        <p><strong>{getCharacterName(dialogue.characterId)}:</strong></p>
                                        <audio src={dialogue.voiceUrl} controls className="h-8" />
                                      </div>
                                    </div>
                                  )
                                ))}
                ...
                <div className="absolute top-1/2 -translate-y-1/2 right-0 translate-x-1/2 flex flex-col space-y-2 p-2 border bg-white rounded-lg">
                  <TooltipProvider>
